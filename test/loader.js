...
tap.test('loader() - Import map defined through option "maps" take precedence over import map defined in eik.json', async (t) => {
    const app = fastify();
    app.server.keepAliveTimeout = 20;
    app.get('/one', (request, reply) => {
        reply.send({
    ...
    t.matchSnapshot(clean(bundle), 'Should rewrite import statement to https://cdn.eik.dev/lit-element/v2');
    await app.close();
    await fs.promises.unlink(path.join(process.cwd(), 'eik.json'));
    t.end();
});

tap.test('loader() - Import map defined through option "urls" argument take precedence over import map defined in eik.json', async (t) => {
    const app = fastify();
    app.server.keepAliveTimeout = 20;
    app.get('/one', (request, reply) => {
        reply.send({
...
tap.test('loader() - Import map defined through option "maps" argument take precedence over import map defined through option "urls" argument', async (t) => {
    const app = fastify();
    app.server.keepAliveTimeout = 20;
    app.get('/one', (request, reply) => {
        reply.send({
